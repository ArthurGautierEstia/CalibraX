...
@dataclass
class ValidityKinematicsSnapshot:
...
def build_validity_context_snapshot(
    robot_model: RobotModel,
    tool_model: ToolModel,
    workspace_model: WorkspaceModel,
) -> ValidityContextSnapshot:
    kinematics_snapshot = ValidityKinematicsSnapshot.from_robot_model(robot_model, tool_model)
    return ValidityContextSnapshot(
        dh_params=kinematics_snapshot.dh_params,
        measured_dh_params=kinematics_snapshot.measured_dh_params,
        measured_dh_enabled=kinematics_snapshot.measured_dh_enabled,
        axis_reversed=list(kinematics_snapshot.axis_reversed),
        corrections=[list(row) for row in kinematics_snapshot.corrections],
        tool_pose=kinematics_snapshot.tool_pose.copy(),
...
class ValidityAnalyzer:
    def __init__(self, context: ValidityContextSnapshot) -> None:
        self.context = context
        self._kinematics = ValidityKinematicsSnapshot(
            dh_params=[list(row) for row in context.dh_params],
            measured_dh_params=[list(row) for row in context.measured_dh_params],
            measured_dh_enabled=bool(context.measured_dh_enabled),
            axis_reversed=list(context.axis_reversed),
            corrections=[list(row) for row in context.corrections],
            tool_pose=context.tool_pose.copy(),
        )
    ...
    def analyze_sample(
        ...
        cancel_token: BuildCancelToken,
    ) -> SampleValidationResult | None:
        ...
        if corrected_matrices is None:
            corrected_matrices = self._kinematics.compute_corrected_matrices(sample.joints)
        if corrected_matrices is None:
            return None
        if cancel_token.is_cancelled():
            return None
        ...
        diagnostics = ValidityAnalyzer._diagnostics_from_pairs(
            self._collision_cache.find_workspace_collisions(),
            TrajectoryCollisionDomain.WORKSPACE,
        )
        diagnostics.extend(
            ValidityAnalyzer._diagnostics_from_pairs(
    ...
    def analyze_task(
        ...
        for entry in task.samples:
            ...
            sample_result = self.analyze_sample(
                entry.sample,
                entry.segment_index,
                entry.sample_index,
        ...
        return ValidationResult(
            revision_id=task.revision_id,
            task_id=task.task_id,
            cancelled=bool(cancel_token.is_cancelled()),
            sample_results=[] if cancel_token.is_cancelled() else sample_results,
        )
...
def apply_validation_result(
    ...
    changed = False
    affected_segment_indices: set[int] = set()
    for sample_result in result.sample_results:
        ...
        sample.error_axis = None
        affected_segment_indices.add(sample_result.segment_index)
        changed = True

    if not changed:
        return False

    for segment_index in affected_segment_indices:
        segment = _segment_at(trajectory, segment_index)
